...
const showAlertPopup = (msg) => {
  el("alertModalBody").textContent = msg;
  new bootstrap.Modal(el("alertModal")).show();
};
...
el("ocrTranslateBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    const data = await res.json();
    if (data.error) { showAlertPopup(data.error); el("outputText").textContent = "Your translation will appear here..."; return; }
    el("sourceLangLabel").textContent = data.detected_lang_name;
...
/* ---------------------------------------------------------------------- */
/* Output actions: copy / download text                                    */
/* ---------------------------------------------------------------------- */
