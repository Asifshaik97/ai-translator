...
| OCR | `pytesseract` + Pillow | Requires the Tesseract binary installed on your OS |
| Document translation | `PyPDF2`, `python-docx` | `.pdf`, `.docx`, `.txt` supported |
| Storage | SQLite | `instance/translator.db`, auto-created on first run |

### Optional: OS-level dependency for OCR
Tesseract must be installed separately (it's a system binary, not a Python package):
